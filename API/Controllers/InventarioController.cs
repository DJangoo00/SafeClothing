...
public class InventarioController : BaseApiController
{
    ...
    public InventarioController(IUnitOfWork unitofwork, IMapper mapper, IUserService Userservice)
    {
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<InventarioDto>>> GetPagination([FromQuery] Params pagparams)
    {
        var entidad = await unitofwork.Inventarios.GetAllAsync(pagparams.PageIndex, pagparams.PageSize, pagparams.Search);
        var listEntidad = mapper.Map<List<InventarioDto>>(entidad.registros);
        return new Pager<InventarioDto>(listEntidad, entidad.totalRegistros, pagparams.PageIndex, pagparams.PageSize, pagparams.Search);
    }
}
