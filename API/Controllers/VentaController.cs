...
public class VentaController : BaseApiController
{
    ...
    public VentaController(IUnitOfWork unitofwork, IMapper mapper, IUserService Userservice)
    {
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<VentaDto>>> GetPagination([FromQuery] Params pagparams)
    {
        var entidad = await unitofwork.Ventas.GetAllAsync(pagparams.PageIndex, pagparams.PageSize, pagparams.Search);
        var listEntidad = mapper.Map<List<VentaDto>>(entidad.registros);
        return new Pager<VentaDto>(listEntidad, entidad.totalRegistros, pagparams.PageIndex, pagparams.PageSize, pagparams.Search);
    }
}
