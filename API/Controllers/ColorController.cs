...
public class ColoresController : BaseApiController
{
    ...
    public ColoresController(IUnitOfWork unitofwork, IMapper mapper, IUserService Userservice)
    {
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<ColorDto>>> GetPagination([FromQuery] Params pagparams)
    {
        var entidad = await unitofwork.Colores.GetAllAsync(pagparams.PageIndex, pagparams.PageSize, pagparams.Search);
        var listEntidad = mapper.Map<List<ColorDto>>(entidad.registros);
        return new Pager<ColorDto>(listEntidad, entidad.totalRegistros, pagparams.PageIndex, pagparams.PageSize, pagparams.Search);
    }
}
