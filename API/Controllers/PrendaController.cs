...
public class PrendaController : BaseApiController
{
    ...
    public PrendaController(IUnitOfWork unitofwork, IMapper mapper, IUserService Userservice)
    {
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<Pager<PrendaDto>>> GetPagination([FromQuery] Params pagparams)
    {
        var entidad = await unitofwork.Prendas.GetAllAsync(pagparams.PageIndex, pagparams.PageSize, pagparams.Search);
        var listEntidad = mapper.Map<List<PrendaDto>>(entidad.registros);
        return new Pager<PrendaDto>(listEntidad, entidad.totalRegistros, pagparams.PageIndex, pagparams.PageSize, pagparams.Search);
    }
}
